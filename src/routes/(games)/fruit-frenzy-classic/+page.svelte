...
<script lang="ts">
  ...
  let slot1:string = icons[giveRandom(icons.slice(0,6))],
      ...
      feedback:string|number = $state("startSpin"), 
      bet:number             = $state(10),
  ...
  // Return random index
  function giveRandom(array:any) { return Math.floor(Math.random() * array.length); }
  ...
  async function calculate(fruits:string[]) {
    ...
    switch (compare(fruit1, fruit2, fruit3)) {
      ...
      case 7:
        ...
        break;
      default:
        feedback = "quips." + quips[giveRandom(quips)];
        break;
    }
    win = gain;
...
<div class="h-screen flex justify-center items-center
  ...
  <div class="pt-2 px-2 bg-red-700 sm:rounded-3xl border-2">
    <h1 class="text-4xl sm:text-5xl text-center font-bold border-b-2 orangek">
      Fruit Frenzy Classic
    </h1>
        
    <!-- Slots -->
    <div class="border flex flex-row items-center my-2 select-none">
    ...
    <!-- Rules modal trigger -->
    <button class="text-center text-wrap hover:underline max-sm:underline orangek text-2xl mx-auto flex " 
            onclick={() => modal = true}>
      {$_("games.rules")}
    </button>
  </div>
