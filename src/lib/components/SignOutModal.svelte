<script>
  let { show = false, clicked = $bindable() } = $props();
    ...
    <div class="flex min-h-full justify-center p-4 text-center items-center sm:p-0">
      <div class="relative transform overflow-hidden border-yellow-600 bg-black text-left border transition-all sm:my-8 max-w-3xl mx-2 p-4 rounded-3xl text-yellow-600">
        ...
        <div class="mt-2 border-t pt-2 border-yellow-600 px-2 max-sm:flex-col-reverse gap-2 flex justify-center font-bold">
          <button class="hover:scale-105 border-2 p-1 rounded bg-black text-yellow-600 border-yellow-600 duration-300" onclick={() => clicked = false}>Close</button>
          <button class="hover:scale-105 border-2 p-1 rounded bg-yellow-600 text-black border-yellow-600 hover:bg-black hover:text-yellow-600 duration-300" onclick={signOut}>Sign out</button>
          <button class="hover:scale-105 border-2 p-1 rounded bg-yellow-600 text-black border-yellow-600 hover:bg-black hover:text-yellow-600 duration-300" onclick={signOutAll}>Sign out everywhere</button>
        </div>
      </div>
    </div>
  </div>
